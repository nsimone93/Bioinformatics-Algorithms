...
void usage(){
	cout<<"scTurtle64 Usage:"<<endl;
	cout <<"scTurtle64 [arguments]"<<endl;
	cout<<"example: ./scTurtle64 -f 1Mreads.fq -o kmer_counts -k 31 -n 6000000 -t 9"<<endl;
	  ...
	  while ((argc > 1) )
		{
			...
			switch (argv[1][1])
			{
				...
				case 'k':
					...
					if (kmer_length>64){
						cout<<"scTurtle64 can handle k-mers of length up to 64."<<endl;
						return 0;
						}
					if (kmer_length<=32){
						cout<<". Please consider using scTurtle32 for k-mers of length <=32. scTurtle32 is more memory and time efficient for smaller k-mers."<<endl;
						}
					break;
				case 'n':
